...
df['开始日期'] = pd.to_datetime(df['开始日期'], errors='coerce').dt.strftime('%Y/%m/%d')
df['结束日期'] = pd.to_datetime(df['结束日期'], errors='coerce').dt.strftime('%Y/%m/%d')
...
for event in events_list:
    ...
    if "具体时间待定" in str(event["描述"]):
        event["开始时间"] = time(23,59).strftime('%H:%M')
        event["结束时间"] = time(23,59).strftime('%H:%M')
