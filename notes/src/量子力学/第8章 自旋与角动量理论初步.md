...
在非耦合表象的基底中，可以验证四个态均为 $\hat{S}_z$ 的本征态，根据 $\hat{S}_z = \hat{S}_{1z} + \hat{S}_{2z}$ 以及 $\hat{S}_{kz} |\uparrow\ \rangle_k = \frac12\hbar\ |\uparrow\ \rangle_k \ ,\ \hat{S}_{kz} |\downarrow\ \rangle_k = -\frac12\hbar\ |\downarrow\ \rangle_k$ ，容易得到
